# פרוקסי ל-RiseUp API (חלופה)

**ברוב המקרים לא צריך את זה.** `server.js` שבשורש הריפו כבר מתווך את הקריאות מאותו origin
שמגיש את הדף — ראה ה-README הראשי. הפרוקסי כאן רלוונטי רק אם מגישים את `index.html`
כקובץ סטטי בלי השרת הזה.
